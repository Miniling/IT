...
클라우드(Cloud) : 네트워크 관리자들 사이에서 인터넷이 '구름처럼' 미지의 공간으로 가려져 있다고 표현하면서 시작됨. </br>
  ...
  - IaaS : Infrastructure as a Service. **사용자가 관리**할 수 있는 범위가 가장 넓은 클라우드 서비스. 컴퓨팅 자원 제공. 서버 운영체제, 미들웨어, 런타임, 데이터, 어플리케이션을 사용자가 직접 구성하고 관리 가능. (ex. Netflix) </br>
  - PaaS : Platform as a Service. IaaS 형태의 클라우드에 사용자가 원하는 서비스를 개발할 수 있도록 **개발 환경**을 미리 구축하여 제공하는 클라우드 서비스. 개발 서비스와 비즈니스에 집중 가능. 관리상 자유도는 낮음. </br>
  - SaaS : Software as a Service. 사용자가 장치에 관계없이 어디에서나 SaaS **어플리케이션**에 로그인할 수 있게 해주는 서비스. 사용자에게 큰 유연성 제공. 장치를 변경해도 다시 설치하거나 라이선스를 새로 구매할 필요x (ex. Google-Gmail 계정으로 구글 서비스 모두 이용 가능) </br>
  - DaaS : Desktop as a Service. **재택 근무**가 가능하도록 환경을 제공해주는 서비스. </br>
  - BaaS : Backend as a Service. 어플리케이션 **개발**에 필요한 백엔드 기능을 클라우드 환경으로 제공하는 서비스. </br>
